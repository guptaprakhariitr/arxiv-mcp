...
const SERVER_INFO = { name: "arxiv-mcp", version: "0.1.0" };
...
const LLMS_TXT = `# arxiv-mcp

> ArXiv preprint search + author-graph for AI agents. Free underlying data.

## Tools
- arxiv_search(query?, author?, category?, sort_by?)
- arxiv_get_paper(id)
...
function renderLanding(env: Env, url: URL): string {
  ...
  const tagline = env.PRODUCT_TAGLINE ?? "MCP server for arXiv preprints \u2014 search, daily digest, author-graph.";
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><title>arxiv-mcp</title>
